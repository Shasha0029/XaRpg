    ...
    set ir=%cd%
      ...
      md %dir%Skills
        set sdir=%dir%\Skills\
          md %sdir%AtkSkills
          md %sdir%BuffSkills
            set asdir=%dir%\Skills\AtkSkills\
            set bsdir=%dir%\Skills\BuffSkills\
    ...
    echo We must fight!
      ...
      if %errorlevel% equ 1 goto TutorialRatYourAtk
        ...
        :TutorialRatEnemyAtk
          ...
          set /a chance=100-%calcspd%+%calcratspd%
          set /a "value = %random% %% 100"
          if %value% GEQ %chance% goto EnemyMiss else (
...
::Battle Section
  ...
  :PreSkele
    Del %dir%BattleID.txt
    Del %mdir%skelehp.txt
    Del %mdir%skeleatk.txt
    Del %mdir%skeledef.txt
    Del %mdir%skelespd.txt
    Echo Skeleton >> %dir%BattleID.txt
    Echo 10 >> %mdir%skelehp.txt
    Echo 4 >> %mdir%skeleatk.txt
    Echo 0 >> %mdir%skeledef.txt
    Echo 3 >> %mdir%skelespd.txt
    cls
  goto Skeleton
  :Skeleton
    for /f "tokens=* delims=," %%G in (%dir%battleid.txt) do set bid=%%G
    ...
    for /f "tokens=* delims=," %%G in (%dir%maxmana.txt) do set maxmana=%%G
    for /f "tokens=* delims=," %%G in (%dir%mana.txt) do set mana=%%G
    for /f "tokens=* delims=," %%G in (%mdir%skelehp.txt) do set skelehp=%%G
    for /f "tokens=* delims=," %%G in (%mdir%skeleatk.txt) do set skeleatk=%%G
    for /f "tokens=* delims=," %%G in (%mdir%skeledef.txt) do set skeledef=%%G
    for /f "tokens=* delims=," %%G in (%mdir%skelespd.txt) do set skelespd=%%G
    cls
      echo/
      echo Enemy: Skeleton		:		Player: %name%
      echo Health: %skelehp%		:		Health: %hp%
      echo Attack: %skeleatk%		:		Attack: %atk%
      echo Defense: %skeledef%		:		Defense: %def%
      echo Speed: %skelespd%		:		Speed: %spd%
      Echo/
        Echo 1: Attack     2:Block     3. Skills     4. Run
      echo/
        choice.exe /C 1234 /N /M "What will you do?"
        ...
        if %errorlevel% equ 3 goto Skill
        if %errorlevel% equ 2 goto Def
        if %errorlevel% equ 1 goto SkeletonYourAtk
      :SkeletonYourAtk
        for /f "tokens=* delims=," %%G in (%dir%spd.txt) do set calcspd=%%G
        for /f "tokens=* delims=," %%G in (%mdir%skelespd.txt) do set calcskelespd=%%G
        set /a chance=100+%calcspd%-%calcskelespd%
        set /a "value = %random% %% 100"
        if %value% GEQ %chance% goto Miss else (
        for /f "tokens=* delims=," %%G in (%dir%atk.txt) do set calcatk=%%G
        for /f "tokens=* delims=," %%G in (%mdir%skeledef.txt) do set calcdef=%%G
        set /a dmg=calcatk-(calcdef/2)
        set/p "val=" <%mdir%skelehp.txt
        set/a "val-=%dmg%"
        (echo(%val%)>%mdir%skelehp.txt
        Echo You attack the Skeleton and deal %dmg%
      :SkeletonEnemyAtk
        for /f "tokens=* delims=," %%G in (%mdir%skelehp.txt) do set number=%%G
        if %number% LEQ 0 goto SkeleDeath else (
        for /f "tokens=* delims=," %%G in (%dir%spd.txt) do set calcspd=%%G
        for /f "tokens=* delims=," %%G in (%mdir%skelespd.txt) do set calcskelespd=%%G
        set /a chance=100-%calcspd%+%calcskelespd%
        set /a "value = %random% %% 100"
        if %value% GEQ %chance% goto EnemyMiss else (
        for /f "tokens=* delims=," %%G in (%mdir%skeleatk.txt) do set calcatk=%%G
        for /f "tokens=* delims=," %%G in (%dir%def.txt) do set calcdef=%%G
        set /a dmg=calcatk-(calcdef/2)
        if %dmg% leq 0 set dmg=1
        set/p "val=" <%dir%hp.txt
        set/a "val-=%dmg%"
        (echo(%val%)>%dir%hp.txt
        Echo The Skeleton deals %dmg% to you!
        for /f "tokens=* delims=," %%G in (%dir%hp.txt) do set phealth=%%G
        if %phealth% LEQ 0 goto Death else (
        @echo.
        pause
  ...
  :PreCult
    Del %dir%BattleID.txt
    Del %mdir%culthp.txt
    Del %mdir%cultatk.txt
    Del %mdir%cultdef.txt
    Del %mdir%cultspd.txt
    Echo Cult >> %dir%BattleID.txt
    Echo 20 >> %mdir%culthp.txt
    Echo 8 >> %mdir%cultatk.txt
    Echo 4 >> %mdir%cultdef.txt
    Echo 6 >> %mdir%cultspd.txt
    Echo 10 >> %mdir%cultmana.txt
    cls
  goto Cult
  :Cult
    for /f "tokens=* delims=," %%G in (%dir%battleid.txt) do set bid=%%G
    ...
    for /f "tokens=* delims=," %%G in (%dir%maxmana.txt) do set maxmana=%%G
    for /f "tokens=* delims=," %%G in (%dir%mana.txt) do set mana=%%G
    for /f "tokens=* delims=," %%G in (%mdir%culthp.txt) do set culthp=%%G
    for /f "tokens=* delims=," %%G in (%mdir%cultatk.txt) do set cultatk=%%G
    for /f "tokens=* delims=," %%G in (%mdir%cultdef.txt) do set cultdef=%%G
    for /f "tokens=* delims=," %%G in (%mdir%cultspd.txt) do set cultspd=%%G
    cls
      echo/
      echo Enemy: Cult		:		Player: %name%
      echo Health: %culthp%		:		Health: %hp%
      echo Attack: %cultatk%		:		Attack: %atk%
      echo Defense: %cultdef%		:		Defense: %def%
      echo Speed: %cultspd%		:		Speed: %spd%
      Echo/
        Echo 1: Attack     2:Block     3. Skills     4. Run
      echo/
        choice.exe /C 1234 /N /M "What will you do?"
        ...
        if %errorlevel% equ 3 goto Skill
        if %errorlevel% equ 2 goto Def
        if %errorlevel% equ 1 goto CultYourAtk
      :CultYourAtk
        for /f "tokens=* delims=," %%G in (%dir%spd.txt) do set calcspd=%%G
        for /f "tokens=* delims=," %%G in (%mdir%cultspd.txt) do set calccultspd=%%G
        set /a chance=100+%calcspd%-%calccultspd%
        set /a "value = %random% %% 100"
        if %value% GEQ %chance% goto Miss else (
        for /f "tokens=* delims=," %%G in (%dir%atk.txt) do set calcatk=%%G
        for /f "tokens=* delims=," %%G in (%mdir%cultdef.txt) do set calcdef=%%G
        set /a dmg=calcatk-(calcdef/2)
        set/p "val=" <%mdir%culthp.txt
        set/a "val-=%dmg%"
        (echo(%val%)>%mdir%culthp.txt
        Echo You attack the Cult and deal %dmg%
      :CultEnemyAtk
        for /f "tokens=* delims=," %%G in (%mdir%culthp.txt) do set number=%%G
        if %number% LEQ 0 goto CultDeath else (
        for /f "tokens=* delims=," %%G in (%dir%spd.txt) do set calcspd=%%G
        for /f "tokens=* delims=," %%G in (%mdir%cultspd.txt) do set calccultspd=%%G
        set /a chance=100-%calcspd%+%calccultspd%
        set /a "value = %random% %% 100"
        if %value% GEQ %chance% goto EnemyMiss else (
        for /f "tokens=* delims=," %%G in (%mdir%cultatk.txt) do set calcatk=%%G
        for /f "tokens=* delims=," %%G in (%dir%def.txt) do set calcdef=%%G
        set /a dmg=calcatk-(calcdef/2)
        if %dmg% leq 0 set dmg=1
        set/p "val=" <%dir%hp.txt
        set/a "val-=%dmg%"
        (echo(%val%)>%dir%hp.txt
        Echo The Cult deals %dmg% to you!
        for /f "tokens=* delims=," %%G in (%dir%hp.txt) do set phealth=%%G
        if %phealth% LEQ 0 goto Death else (
        @echo.
        pause
...
::Battle Related Section
  ...
  :DRatDeath
    ...
    If exist "%asdir%\Test. " (
    goto %save%)
    set /a "schance = %random% %% 100"
    if %schance% GEQ 6 goto %save% else (
    Echo You have unlocked SKILLNAME!
    Echo 1 >>%asdir%Test.
    pause
  goto %save%
  :SkeleDeath
    echo Skeleton was slain like a bitch
    pause
    Del %mdir%skelehp.txt
    Del %mdir%skeleatk.txt
    Del %mdir%skeledef.txt
  ...
  goto %save%
  :CultDeath
    echo Cultist was slain like a bitch
    pause
    Del %mdir%culthp.txt
    Del %mdir%cultatk.txt
    Del %mdir%cultdef.txt
...
::Skill Related Section
  ...
  if %bid% Equ Skeleton goto SkeletonEnemyAtk
  if %bid% Equ Cult goto CultEnemyAtk
